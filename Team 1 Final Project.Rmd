...
## Basic Information About Cardiovascular Diseases (CVDs)
...
## Reasons of Choosing Cardiovascular Disease.
...
### Are there any common in CVD patients?
...
### Plot comparing different factors
...
sctest
```
Since p-value = `r format(sctest$p.value, scientific = T)` < 0.1, we reject the null hypothesis, so `smoke` has impact on `cardio`.   
...
alctest
```
Since p-value = `r format(alctest$p.value, scientific = T` < 0.1, we reject the null hypothesis, so `alco` has impact on `cardio`.   
...
act_test
```
Since p-value = `r format(act_test$p.value, scientific = T` < 0.1, we reject the null hypothesis, so `active` has an impact on `cardio`.


## Continuous Variables EDA to Cardiovascular Diseases (CVDs)
...
## Will Age influence the CVD rate with other variables?
...
## Will Height influence the CVD rate with other variables?
...
## Will Weight influence the CVD rate with other variables?
...
### Feature Selection and Depth Selection   
